...
module.exports = async (client, channelID) => {
  ...
  Promise.all(
    ticketMessages.map(async msg => {
      return new Promise(async (resolve, reject) => {
        msg.fields = await Utils.DB.tickets.ticket_messages.getEmbedFields(msg.message);
        resolve();
      })
    })
  )
  ...
  function replace(text) {
    return text
      ...
      .replace(regexs.user, user => {
        const userFound = guild.member(user.match(/\d+/) ? user.match(/\d+/)[0] : '');
        return userFound ? "@" + usersFound.user.tag : user;
      })
  ...
  const html = `<!DOCTYPE html>
    <html>
      ...
      <body>
        <div class="container">
          <p class="server-name">${client.guilds.cache.get(ticket.guild).name}</h1>
